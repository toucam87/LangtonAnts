...
@onready var ants_manager = $AntsManager
...
#	if ants_manager.is_placing_ants:
#		if event.is_action_pressed("left click"):
#			var coordinate = board.local_to_map(board.to_local(Mouse.mouse_pos)) 
#			if board.is_tile_in_map(coordinate):
#				ants_manager.pawn_ant()
#				ants_manager.place_ant(ants_manager.ants.back(), coordinate, ants_manager.orientation_of_ant_to_place)
#		elif event.is_action_pressed("right click"):
#			var coordinate = board.local_to_map(board.to_local(Mouse.mouse_pos)) 
#			var ants_clicked = ants_manager.ants_present(coordinate)
#			if ants_clicked.size() > 0:
#				for ant in ants_clicked:
#					ants_manager.destroy_ant(ant)
